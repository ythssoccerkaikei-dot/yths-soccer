  ...
  return (
    <div className="min-h-screen bg-gray-50">
              ...
              {currentUser.role !== 'accounting' ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-2 rounded-lg transition-colors text-xs sm:text-base ${
                      currentView === 'report' ? 'bg-blue-100 text-blue-700' : 'text-gray-700 hover:bg-gray-100'
                    }`}
                  >
                    <FileText size={16} className="sm:w-[18px] sm:h-[18px]" />
                    <span className="hidden md:inline">会計報告</span>
                  </button>
                  <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* 会計報告画面 */}
        {currentView === 'report' && currentUser.role === 'accounting' && (
          <>
            {/* 印刷用スタイル */}
            <style>{`
              @media print {
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-lg shadow">
                <div className="p-6 border-b border-gray-200 no-print">
                  <div className="flex justify-between items-center">
                    <div>
                      <h2 className="text-lg font-semibold text-gray-800">会計報告</h2>
                      <p className="text-sm text-gray-600 mt-1">年度ごとの収支報告書を作成します</p>
                    </div>
                    <select
                      value={selectedFiscalYear}
                      onChange={(e) => setSelectedFiscalYear(e.target.value)}
                ...
                <div className="p-8" id="report-content" style={{fontFamily: 'sans-serif'}}>
                  {(() => {
                    const fiscalYear = fiscalYears[selectedFiscalYear];
                    if (!fiscalYear) return null;
                    
                    // 会費収入の計算
                    const fiscalMembers = members.filter(m => m.fiscalYear === selectedFiscalYear);
                    let membershipFeeTotal = 0;
                    fiscalMembers.forEach(member => {
                      getFiscalYearMonths().forEach(m => {
                      const monthKey = `${member.id}:${m.year}-${String(m.month + 1).padStart(2, '0')}`;
                      if (membershipFees[monthKey]) {
                        membershipFeeTotal += member.monthlyFee;
                      }
                    });
                  });
                  
                  // 会費以外の収入
                  const otherIncomes = incomes.filter(i => i.fiscalYear === selectedFiscalYear);
                  const otherIncomeTotal = otherIncomes.reduce((sum, i) => sum + i.amount, 0);
                  
                  // 総収入
                  const totalIncome = membershipFeeTotal + otherIncomeTotal;
                  ...
                  const totalExpense = fiscalExpenses.reduce((sum, e) => sum + e.amount, 0);
                  ...
                  return (
                    <div className="space-y-6">
                      ...
                      <div className="text-center border-b-2 border-gray-300 pb-4">
                        <h3 className="text-2xl font-bold text-gray-800">{fiscalYear.name} 会計報告書</h3>
                        <p className="text-sm text-gray-600 mt-2">
                          期間: {fiscalYear.startYear}年{fiscalYear.startMonth + 1}月 〜 {fiscalYear.endYear}年{fiscalYear.endMonth + 1}月
                        </p>
                      </div>
                      
                      {/* 収入の部 */}
                      <div>
                        <h4 className="text-lg font-semibold text-gray-800 mb-3 bg-green-50 p-2 rounded">収入の部</h4>
                        <div className="space-y-2">
                          <div className="flex justify-between items-center p-3 border-b border-gray-200">
                            <span className="font-medium">会費収入</span>
                            <span className="text-lg font-bold text-green-600">
                              ¥{membershipFeeTotal.toLocaleString()}
                            </span>
                          </div>
                          {otherIncomes.length > 0 && (
                            <div className="pl-4 space-y-1">
                              {otherIncomes.map(income => {
                                const [year, monthNum] = income.month.split('-');
                                return (
                                  <div key={income.id} className="flex justify-between text-sm text-gray-600">
                                    <span>{year}年{monthNum}月 - {income.categoryName} {income.description && `(${income.description})`}</span>
                                    <span>¥{income.amount.toLocaleString()}</span>
                                  </div>
                                );
                              })}
                              <div className="flex justify-between items-center p-3 border-b border-gray-200 font-medium">
                                <span>会費以外の収入</span>
                                <span className="text-lg font-bold text-green-600">
                          ...
                          <div className="flex justify-between items-center p-4 bg-green-100 rounded-lg font-bold text-lg">
                            <span>収入合計</span>
                      ...
                      {/* 支出の部 */}
                      <div>
                        <h4 className="text-lg font-semibold text-gray-800 mb-3 bg-red-50 p-2 rounded">支出の部</h4>
                        <div className="space-y-2">
                          {Object.entries(expensesByCategory).map(([category, amount]) => (
                            <div key={category} className="flex justify-between items-center p-3 border-b border-gray-200">
                          ...
                          {Object.keys(expensesByCategory).length === 0 && (
                            <div className="text-center py-4 text-gray-500">支出がありません</div>
                          )}
                          <div className="flex justify-between items-center p-4 bg-red-100 rounded-lg font-bold text-lg">
                            <span>支出合計</span>
                      ...
                      <div className="border-t-4 border-gray-300 pt-4">
                        <div className={`flex justify-between items-center p-6 rounded-lg font-bold text-2xl ${
                          balance >= 0 ? 'bg-blue-100 text-blue-900' : 'bg-red-100 text-red-900'
                        }`}>
                          <span>残高</span>
                      ...
                      <div className="mt-6 p-4 bg-blue-50 rounded-lg text-sm text-blue-800">
                        <p className="font-medium mb-2">PDF出力方法：</p>
                        <ol className="list-decimal list-inside space-y-1">
                          <li>「PDF出力（印刷）」ボタンをクリック</li>
                          <li>印刷画面で「送信先」を「PDFに保存」を選択</li>
                          <li>「保存」をクリックしてPDFファイルを保存</li>
                        </ol>
                      </div>
                    </div>
                  );
                  ...
                  <>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full border-collapse">
                        ...
                        <tbody>
                          {members
                            .filter(m => m.fiscalYear === selectedFiscalYear && m.active)
                ...
                <div>
                  ...
                  {(() => {
                    const fiscalIncomes = incomes
                      ...
                      .sort((a, b) => {
                        // month形式(YYYY-MM)で比較
                        const monthA = a.month || a.date?.substring(0, 7) || '';
                        const monthB = b.month || b.date?.substring(0, 7) || '';
                    ...
                    return (
                      <div className="space-y-2">
                        {fiscalIncomes.map(income => {
                          const month = income.month || income.date?.substring(0, 7) || '';
                  ...
                  <div>
                    ...
                    {/* 年度追加フォーム */}
                      <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg">
                        <h4 className="font-medium text-gray-800 mb-3">新しい年度を追加</h4>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                          <input
                            type="text"
                            placeholder="年度名（例: 2025年度）"
                            className="px-4 py-2 border border-gray-300 rounded-lg"
                            id="new-fiscal-year-name"
                          />
                          <div>
                            <label className="block text-xs font-medium text-gray-700 mb-1">開始年月</label>
                          ...
                          <div>
                            <label className="block text-xs font-medium text-gray-700 mb-1">終了年月</label>
                            <div className="flex gap-2">
                              <input
                                type="number"
                                placeholder="年"
                        ...
                        <button
                          onClick={() => {
                            const name = document.getElementById('new-fiscal-year-name').value.trim();
                            const startYear = parseInt(document.getElementById('new-fiscal-year-start-year').value);
                            const startMonth = parseInt(document.getElementById('new-fiscal-year-start-month').value);
                            const endYear = parseInt(document.getElementById('new-fiscal-year-end-year').value);
                            const endMonth = parseInt(document.getElementById('new-fiscal-year-end-month').value);
                            ...
                            setFiscalYears({...fiscalYears, [newId]: newFiscalYear});
                            setSelectedFiscalYear(newId);
                            
                            // フォームをクリア
                            document.getElementById('new-fiscal-year-name').value = '';
                            document.getElementById('new-fiscal-year-start-year').value = new Date().getFullYear();
                            document.getElementById('new-fiscal-year-start-month').value = '3';
                            document.getElementById('new-fiscal-year-end-year').value = new Date().getFullYear() + 1;
                            document.getElementById('new-fiscal-year-end-month').value = '2';
                                ...
                                {!isEditing ? (
                                  ...
                                  <div className="mt-3 space-y-3">
                                    <div className="grid grid-cols-2 gap-3">
                                      <div>
                                        ...
                                        <div className="flex gap-2">
                                          <input
                                            type="number"
                                            id={`start-year-${year.id}`}
                                            defaultValue={year.startYear}
                                            className="w-20 px-2 py-1 border border-gray-300 rounded text-sm"
                                            placeholder="年"
                                          />
                                          <select
                                            id={`start-month-${year.id}`}
                                            defaultValue={year.startMonth}
                                      ...
                                      <div>
                                        ...
                                        <div className="flex gap-2">
                                          <input
                                            type="number"
                                            id={`end-year-${year.id}`}
                                            defaultValue={year.endYear}
                                            className="w-20 px-2 py-1 border border-gray-300 rounded text-sm"
                                            placeholder="年"
                                          />
                                          <select
                                            id={`end-month-${year.id}`}
                                            defaultValue={year.endMonth}
                                            className="flex-1 px-2 py-1 border border-gray-300 rounded text-sm"
                                          >
                                            {[...Array(12)].map((_, i) => (
                                              <option key={i} value={i}>{i + 1}月</option>
                                    ...
                                    <div className="flex gap-2">
                                      <button
                                        onClick={() => {
                                          const startYear = parseInt(document.getElementById(`start-year-${year.id}`).value);
                                          const startMonth = parseInt(document.getElementById(`start-month-${year.id}`).value);
                                          const endYear = parseInt(document.getElementById(`end-year-${year.id}`).value);
                                          const endMonth = parseInt(document.getElementById(`end-month-${year.id}`).value);
                                          
                                          const newFiscalYears = { ...fiscalYears };
                                          newFiscalYears[year.id] = {
                                            ...newFiscalYears[year.id],
                                            startYear,
                                            startMonth,
                                            endYear,
                  ...
                  <div>
                    ...
                    {(() => {
                      ...
                      const prevFiscalYear = Object.values(fiscalYears).find(fy => 
                        fy.endYear === currentFiscalYear.startYear && 
                        fy.endMonth === currentFiscalYear.startMonth
                      );
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                        ...
                        <button
                          onClick={() => {
                            ...
                            const monthlyFee = parseInt(document.getElementById('new-member-monthly-fee').value) || 8000;
                            ...
                            document.getElementById('new-member-monthly-fee').value = '8000';
                          }}
                          className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                        >
                          追加
