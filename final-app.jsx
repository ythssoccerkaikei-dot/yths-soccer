...
// Lucide Icons - React.createElementでラップする関数として定義
const createIcon = (iconName) => {
  return (props) => {
    const IconComponent = window.lucide[iconName];
    if (!IconComponent) {
...
// localStorage版
// Firebase不使用
const { collection, doc, getDoc, setDoc, onSnapshot} = window.firebaseFirestore;
...
const App = () => {
  ...
  const saveAttendances = (data) => {
    localStorage.setItem('attendances', JSON.stringify(data));
  };
  
  const saveAllAttendances = (data) => {
    localStorage.setItem('allAttendances', JSON.stringify(data));
  };
  ...
  useEffect(() => {
    ...
    // ユーザー情報の同期
    const usersUnsub = onSnapshot(doc(db, 'settings', 'users'), (docSnap) => {
      if (docSnap.exists()) {
        setUsers({ ...INITIAL_USERS, ...docSnap.data() });
      }
    }, (error) => console.error('ユーザー同期エラー:', error));
    unsubscribers.push(usersUnsub);

    // 会場情報の同期
    const venuesUnsub = onSnapshot(doc(db, 'settings', 'venues'), (docSnap) => {
      if (docSnap.exists() && docSnap.data().list) {
        setVenues(docSnap.data().list);
      }
    }, (error) => console.error('会場同期エラー:', error));
    unsubscribers.push(venuesUnsub);

    // 年度情報の同期
    const fiscalYearsUnsub = onSnapshot(doc(db, 'settings', 'fiscalYears'), (docSnap) => {
      if (docSnap.exists()) {
        const years = docSnap.data();
        setFiscalYears(years);
        if (!selectedFiscalYear && Object.keys(years).length > 0) {
          setSelectedFiscalYear(Object.keys(years)[0]);
        }
      }
    }, (error) => console.error('年度同期エラー:', error));
    unsubscribers.push(fiscalYearsUnsub);

    // 距離設定の同期
    const distancesUnsub = onSnapshot(doc(db, 'settings', 'distances'), (docSnap) => {
      if (docSnap.exists()) {
        setDistances(docSnap.data());
      }
    }, (error) => console.error('距離同期エラー:', error));
    unsubscribers.push(distancesUnsub);

    // 明細情報の同期
    const statementsUnsub = onSnapshot(doc(db, 'data', 'statements'), (docSnap) => {
      if (docSnap.exists()) {
        setStatements(docSnap.data());
      }
    }, (error) => console.error('明細同期エラー:', error));
    unsubscribers.push(statementsUnsub);

    // 収入記録の同期
    const incomesUnsub = onSnapshot(doc(db, 'data', 'incomes'), (docSnap) => {
      if (docSnap.exists() && docSnap.data().list) {
        setIncomes(docSnap.data().list);
      }
    }, (error) => console.error('収入同期エラー:', error));
    unsubscribers.push(incomesUnsub);

    // 支出記録の同期
    const expensesUnsub = onSnapshot(doc(db, 'data', 'expenses'), (docSnap) => {
      if (docSnap.exists() && docSnap.data().list) {
        setExpenses(docSnap.data().list);
      }
    }, (error) => console.error('支出同期エラー:', error));
    unsubscribers.push(expensesUnsub);

    // 会員情報の同期
    const membersUnsub = onSnapshot(doc(db, 'data', 'members'), (docSnap) => {
      if (docSnap.exists() && docSnap.data().list) {
        setMembers(docSnap.data().list);
      }
    }, (error) => console.error('会員同期エラー:', error));
    unsubscribers.push(membersUnsub);

    // 会費記録の同期
    const feesUnsub = onSnapshot(doc(db, 'data', 'membershipFees'), (docSnap) => {
      if (docSnap.exists()) {
        setMembershipFees(docSnap.data());
      }
    }, (error) => console.error('会費同期エラー:', error));
    unsubscribers.push(feesUnsub);

    // カテゴリーの同期
    const categoriesUnsub = onSnapshot(doc(db, 'settings', 'categories'), (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        if (data.income) setIncomeCategories(data.income);
        if (data.expense) setExpenseCategories(data.expense);
      }
    }, (error) => console.error('カテゴリー同期エラー:', error));
  ...
  useEffect(() => {
    ...
    if (savedDistances) {
      setDistances(JSON.parse(savedDistances));
    }
    
    const savedFiscalYears = localStorage.getItem('fiscalYears');
    if (savedFiscalYears) {
      setFiscalYears(JSON.parse(savedFiscalYears));
    }
    ...
    const savedMembershipFees = localStorage.getItem('membershipFees');
    if (savedMembershipFees) {
      setMembershipFees(JSON.parse(savedMembershipFees));
    }
    ...
    if (currentUser.role === 'accounting') {
      ...
      for (const username of Object.keys(users)) {
        ...
        if (user.role !== 'accounting') {
          try {
            const docRef = doc(db, 'attendances', username);
            const docSnap = await getDoc(docRef);
            if (docSnap.exists()) {
              allData[username] = docSnap.data();
            }
          } catch (error) {
            console.error(`${username}の出席データ読み込みエラー:`, error);
          }
        }
      }
      ...
      try {
        const docRef = doc(db, 'attendances', currentUser.username);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setAttendances(docSnap.data());
        }
      } catch (error) {
  ...
  const saveUsers = async (data) => {
    try {
      await setDoc(doc(db, 'settings', 'users'), data);
    } catch (error) {
  ...
  const saveVenues = async (data) => {
    try {
      await setDoc(doc(db, 'settings', 'venues'), { list: data });
    } catch (error) {
  ...
  const saveFiscalYears = async (data) => {
    try {
      await setDoc(doc(db, 'settings', 'fiscalYears'), data);
    } catch (error) {
  ...
  const saveDistances = async (data) => {
    try {
      await setDoc(doc(db, 'settings', 'distances'), data);
    } catch (error) {
      console.error('距離保存エラー:', error);
    }
  };

  const saveStatements = async (data) => {
    try {
      await setDoc(doc(db, 'data', 'statements'), data);
    } catch (error) {
  ...
  const saveIncomes = async (data) => {
    try {
      await setDoc(doc(db, 'data', 'incomes'), { list: data });
    } catch (error) {
  ...
  const saveExpenses = async (data) => {
    try {
      await setDoc(doc(db, 'data', 'expenses'), { list: data });
    } catch (error) {
  ...
  const saveMembers = async (data) => {
    try {
      await setDoc(doc(db, 'data', 'members'), { list: data });
    } catch (error) {
      console.error('会員保存エラー:', error);
    }
  };

  const saveMembershipFees = async (data) => {
    try {
      await setDoc(doc(db, 'data', 'membershipFees'), data);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
              ...
              {currentUser.role !== 'accounting' ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-2 rounded-lg transition-colors text-xs sm:text-base ${
                      currentView === 'payments' ? 'bg-blue-100 text-blue-700' : 'text-gray-700 hover:bg-gray-100'
                    }`}
                  >
                    <Check size={16} className="sm:w-[18px] sm:h-[18px]" />
                    <span className="hidden md:inline">支払</span>
                  </button>
                  <button
      ...
      saveStatements(newStatements);
        ...
        {/* 支払処理画面 */}
        {currentView === 'payments' && currentUser.role === 'accounting' && (
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow">
              <div className="p-6 border-b border-gray-200">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">確認済みの明細をまとめて支払処理します</p>
                  </div>
                  <select
                    value={selectedFiscalYear}
                    onChange={(e) => setSelectedFiscalYear(e.target.value)}
              ...
              <div className="p-6">
                {(() => {
                  ...
                  getFiscalYearMembers().forEach(username => {
                    const user = users[username];
                    if (!user) return;
                    
                    getFiscalYearMonths().forEach(m => {
                      const monthKey = `${m.year}-${String(m.month + 1).padStart(2, '0')}`;
                      const statementKey = `${username}:${monthKey}`;
                      const statement = statements[statementKey];
                      
                      if (statement?.status === 'confirmed') {
                        const statementData = statement.data || calculateStatement(username, m.year, m.month);
                        if (statementData && statementData.details.length > 0) {
                  ...
                  if (confirmedStatements.length === 0) {
                    return (
                      <div className="text-center py-12 text-gray-500">
                        <p className="mb-2">支払待ちの明細はありません</p>
                        <p className="text-sm">確認済みの明細が表示されます</p>
                      </div>
                    );
                  }

                  const totalAmount = confirmedStatements.reduce((sum, s) => sum + s.data.summary.grandTotal, 0);
                  ...
                  return (
                    <>
                      ...
                      <div className="mb-6 p-6 bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg">
                        <div className="flex justify-between items-center">
                          <div>
                            <h3 className="text-lg font-semibold text-gray-800 mb-2">支払待ち明細</h3>
                            <p className="text-sm text-gray-600">
                              {confirmedStatements.length}件の明細が支払待ちです
                            </p>
                          </div>
                          <div className="text-right">
                            <div className="text-sm text-gray-600 mb-1">支払総額</div>
                      ...
                      <div className="space-y-4 mb-6">
                        {confirmedStatements.map(item => (
                          <div key={item.statementKey} className="border-2 border-gray-200 rounded-lg overflow-hidden">
                            <div className="bg-gray-50 p-4 flex justify-between items-center">
                              <div>
                                <h4 className="font-semibold text-gray-800">
                                  {item.user.name} - {item.month}
                                </h4>
                                <p className="text-sm text-gray-600 mt-1">
                                  {item.data.details.length}件の活動
                                </p>
                              </div>
                              <div className="text-right">
                                <div className="text-2xl font-bold text-purple-600">
                                  ¥{item.data.summary.grandTotal.toLocaleString()}
                                </div>
                                <div className="text-xs text-gray-500 mt-1">
                                  報酬 ¥{item.data.summary.totalRate.toLocaleString()} + 
                                  交通費 ¥{item.data.summary.totalTransport.toLocaleString()} + 
                                  ETC ¥{item.data.summary.totalETC.toLocaleString()}
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="p-4 bg-white">
                              <button
                                onClick={() => setExpandedMonths({
                                  ...expandedMonths,
                                  [item.statementKey]: !expandedMonths[item.statementKey]
                                })}
                                className="text-sm text-blue-600 hover:text-blue-800 mb-2"
                              >
                                {expandedMonths[item.statementKey] ? '▼ 詳細を非表示' : '▶ 詳細を表示'}
                              </button>

                              {expandedMonths[item.statementKey] && (
                                <div className="mt-2 overflow-x-auto">
                                  <table className="w-full text-sm">
                                    <thead className="bg-gray-50">
                                      <tr>
                                        <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">日付</th>
                                        <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">種類</th>
                                        <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">会場</th>
                                        <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">報酬</th>
                                        <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">交通費</th>
                                        <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">ETC</th>
                                        <th className="px-3 py-2 text-right text-xs font-medium text-gray-500">合計</th>
                                      </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-200">
                                      {item.data.details.map((detail, idx) => (
                                        <tr key={idx}>
                                          ...
                                          <td className="px-3 py-2 text-gray-900">{detail.venue}</td>
                      ...
                      <div className="p-6 bg-purple-50 rounded-lg">
                        <div className="flex justify-between items-center">
                          <div>
                            <h4 className="font-semibold text-gray-800 mb-1">支払処理を実行</h4>
                            <p className="text-sm text-gray-600">
                              {confirmedStatements.length}件の明細を支払済みにします
                            </p>
                          </div>
                          <button
                            onClick={() => {
                              if (confirm(`${confirmedStatements.length}件の明細を支払済みにしてもよろしいですか？\n\n支払総額: ¥${totalAmount.toLocaleString()}`)) {
                                const newStatements = { ...statements };
                                confirmedStatements.forEach(item => {
                                  newStatements[item.statementKey] = {
                                    ...newStatements[item.statementKey],
                                    status: 'paid',
                                    paidAt: new Date().toISOString()
                                  };
      ...
      saveStatements(newStatements);
        ...
        {currentView !== 'calendar' && currentView !== 'dashboard' && currentView !== 'incomes' && currentView !== 'expenses' && currentView !== 'membership-fees' && currentView !== 'payments' && (
        <div className="bg-white rounded-lg shadow">
      ...
      saveFiscalYears(newFiscalYears);
                        ...
                        <div className="flex gap-3">
                          ...
                          <button
                            onClick={() => {
                              ...
                              const updatedIncomeCategories = [...incomeCategories, newCategory];
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (confirm(`「${cat.name}」を削除してもよろしいですか？`)) {
                                  const updatedIncomeCategories = incomeCategories.filter(c => c.id !== cat.id);
                  setIncomeCategories(updatedIncomeCategories);
                  saveCategories();
                                }
                              }}
                        ...
                        <div className="flex gap-3">
                          ...
                          <button
                            onClick={() => {
                              ...
                              const updatedExpenseCategories = [...expenseCategories, newCategory];
              ...
              saveCategories();
                          ...
                          <div key={cat.id} className={`border-2 border-${cat.color}-200 bg-${cat.color}-50 rounded-lg p-3 flex justify-between items-center`}>
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (confirm(`「${cat.name}」を削除してもよろしいですか？`)) {
                                  const updatedExpenseCategories = expenseCategories.filter(c => c.id !== cat.id);
                  setExpenseCategories(updatedExpenseCategories);
                  saveCategories();
                                }
                              }}
                ...
                {settingsTab === 'members' && (
                  <div>
                    <h3 className="text-lg font-semibold text-gray-800 mb-4">会員管理</h3>
                    ...
                    <div className="space-y-2">
                      {members.length === 0 ? (
                        ...
                        members
                          .sort((a, b) => a.grade - b.grade || a.name.localeCompare(b.name))
                          .map(member => (
                            <div key={member.id} className="border-2 border-gray-200 rounded-lg p-4 flex justify-between items-center">
                              <div className="flex items-center gap-4">
                                <div>
                                  <h4 className="font-semibold text-gray-800">{member.name}</h4>
                                  <p className="text-sm text-gray-600">
                                    {member.grade}年 - 月額 ¥{member.monthlyFee.toLocaleString()}
                                  </p>
                                </div>
                                {!member.active && (
